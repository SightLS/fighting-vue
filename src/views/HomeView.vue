...
export default class HomeView extends Vue {
  ...
  readonly POINT_SPACING = 20;
  readonly BALL_SPEED = 5;
  ...
  generateSwordPoints(playerIndex: number, isUppercut: boolean) {
    ...
    if (isUppercut) {
      ...
      angle = (player.facingLeft ? 250 : -250) * progressFraction;
    } else {
  ...
  updateEnergyBalls() {
    this.balls = this.balls.filter(ball => {
      ...
      return ball.x >= -this.BALL_SIZE && ball.x <= 1000 + this.BALL_SIZE;
    });
  }
  ...
  isPointCollidingWithBall(point: SwordPoint, ball: Ball): boolean {
    ...
    const distance = Math.sqrt(dx * dx + dy * dy);
    return distance <= this.BALL_SIZE / 2;
  }
